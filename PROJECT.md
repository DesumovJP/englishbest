...
- **2026-04-28** Content-lifecycle plan + admin moderation phase. Новий root-doc **`CONTENT_LIFECYCLE_PLAN.md`** (~1000 рядків) — детальний аудит системи створення/редагування/публікації для уроку/курсу/словника + дизайн approval-workflow (teacher proposes → admin approves) + admin direct-publish path + 11-фазний rollout. **Vocab** виділена окрема секція (§4) з констатацією: default Strapi controller, нуль scoping — будь-який STAFF може редагувати/видалити будь-який vocab-set. Запропоновано: додати `owner` (teacher-profile m2o), `source` (enum platform/own/copy/template), `reviewStatus` (enum draft/submitted/approved/rejected), `originalVocabularySet` (self m2o); замінити default controller scoped-варіантом по аналогії з lesson; FE — фільтр власності + status badge + clone-flow. **`ADMIN_PRODUCTION_PLAN.md`** оновлено — додано Phase 2.5 «Content moderation workflow» між audit lifecycle і user mgmt, з посиланням на новий doc; gap matrix у §2.2 розширено 4 пунктами (3a-3d). **Backend** новий helper `backend/src/lib/audit.ts` з `writeAudit(strapi, ctx, payload)` — best-effort (swallow errors, warn-log), фундамент для Phase 2 (audit lifecycle integration) + Phase L5 (approval-workflow audit). Жодних schema-changes — лише planning + helper module. Backend `tsc --noEmit` чистий.
- **2026-04-28** Course CRUD + vocab editor + lesson-card cleanup (бібліотека-консолідація follow-up). **Backend** новий `api::course.course` controller з `publish`/`unpublish` методами (mirror lesson) + `routes/publish.ts`. **Frontend** proxies `app/api/courses/[id]/{publish,unpublish}/route.ts`. `lib/teacher-courses.ts` додає `createTeacherCourse` (slug = derived-from-title + timestamp suffix; kind='course', audience='any' за замовчуванням), `publishCourse`, `unpublishCourse`, `deleteTeacherCourse`. `CourseSummary/Detail.published: boolean` derived з `publishedAt`; LIST_QUERY + DETAIL_QUERY додають `fields=publishedAt` + `status=draft`. **CoursesTab** — кнопка `+ Курс` → `CreateCourseModal` (title EN/UA + level) → редирект у редактор; список показує `Чернетка` chip для unpublished. **Course editor** — header actions: `Опублікувати`/`Зняти з публікації` + `Видалити`. UI поля `Аудиторія` та `Емоджі` прибрано (audience завжди `'any'`); cover image — `<WipSection>` placeholder («Завантаження картинки — у розробці»). **Lesson cards** (LessonsTab) — прибрано `Копія` + `✕ Видалити` з grid/list, лишилась тільки `Призначити`. Видалення/клонування — тільки на сторінці редактора уроку. **VocabularyTab** cards тепер Link-wrap на `/dashboard/vocabulary/[id]/edit` + `Призначити` button (toast «у розробці» — vocab-as-homework окремий чанк). **New `app/dashboard/vocabulary/[id]/edit/page.tsx`** — повний редактор: title (EN/UA), level, topic, description, inline words list (word + translation + part-of-speech + optional example/exampleTranslation з reorder + remove); Save + Delete у shell actions. `lib/teacher-vocabulary.ts` додає `fetchVocabSet`, `updateVocabSet`, `deleteVocabSet`, `VocabSetDetail` shape + DETAIL_QUERY. **Lesson editor** — course-info fetch винесено з load-effect у окремий effect на `parentLink` state, щоб slow/failed course fetch не блокував редактор від interactive стану.
- **2026-04-28** Read-switch (stage C.1) — `lib/normalize.ts.normalizeCourse` + `lib/teacher-courses.ts.normalizeDetail` тепер можуть виводити `section.lessonSlugs[]` з populated `lessons` relation (group by `sectionSlug`, sort by `orderIndex`). **Пріоритет: stored component `lessonSlugs[]` first**, fallback на relation тільки коли stored порожній — безпечно для не-backfilled курсів, public-read scoping (anonymous лесон-controller повертає тільки platform/template), і draft/publish нюансів на `lesson.course` (Strapi v5 update писав би в draft, public reads брали б published). `COURSE_POPULATE` (`lib/api.ts`) + teacher `DETAIL_QUERY` розширено `populate[lessons][fields]=slug,sectionSlug,orderIndex`. Public-pages (`/library`, `/library/[programSlug]`, `/(kids)/kids/library/[id]`) і course editor читають той самий derived `lessonSlugs[]` — жодних змін у consumer code. Stage C.2 (flip priority + drop schema field + dual-write code) лишається deferred — спрацює лише після того, як backfill виконаний на prod і derive shape підтверджено в логах/QA.
- **2026-04-28** Lesson↔course relation alignment (stage A — dual-write + backfill script). Розв'язує дуальність `course.sections[].lessonSlugs[]` (component) ↔ `lesson.{course, sectionSlug, orderIndex}` (relation+scalars), яка лишилась після library-consolidation. **Frontend** `LessonInput` (`lib/teacher-library.ts`) розширено optional `course/sectionSlug/orderIndex` (+ `title` зроблено optional, бо relation-only-патч); `toPayload` пробрасує. Course editor `handleSaveSections` тепер dual-writes: спершу `updateCourseSections` (як було), потім `propagateLessonRelations` діфає старий vs новий slug→{section,order} map і викликає `updateLesson` для кожного власного (own/copy) уроку, що змінив прив'язку (платформенні/template скіпаються — backend 403 inevitable, swallowed silently). Після save рефрешиться `library` cache, щоб наступне відкриття picker-а бачило оновлений `courseDocumentId/sectionSlug`. **Backend** новий `scripts/backfill-lesson-course-relations.ts` + npm-скрипт `backfill-lesson-courses` (idempotent, link-only — не чистить orphan relations): сканує всі курси, будує slug→target map (dedup-warns на дублі), для кожного знайденого уроку звіряє з поточним `course/sectionSlug/orderIndex` і пише diff. Не запускати на проді без staging-перевірки + бекапу. **Stage B (drop `lessonSlugs[]`)** залишається свідомо відкладеним: треба переключити 4+ public-read paths (`lib/normalize.ts`, `app/library/[slug]/page.tsx`, `app/(kids)/kids/library/[id]/page.tsx`, etc.) на derive-from-relation, потім видалити поле в schema.json + redeploy. Окремий chunk після того як backfill пройшов і верифікований.
- **2026-04-28** Library consolidation. Зливаємо `/dashboard/courses` + `/dashboard/teacher-library` в один хаб `/dashboard/library` із табами (Courses/Lessons/Vocabulary) — каркас на майбутнє розширення (worksheets/reading/audio). **Frontend** новий `app/dashboard/library/page.tsx` (тонкий tab-switcher на `?tab=` з URL-state, спільний search-input у toolbar slot DashboardPageShell, лічильники в табах) + три tab-компоненти в `_components/`: `CoursesTab` (список з `fetchTeacherCourses`), `LessonsTab` (повний UX з `/dashboard/teacher-library` — grid/list, фільтри source+level, +Урок, копія/призначити/видалити), `VocabularyTab` (новий — `fetchAllVocabSets` + scope-фільтр all/course/lesson/standalone, ScopeBadge лінкує до батьківського курсу/уроку). Старі `/dashboard/courses/page.tsx` і `/dashboard/teacher-library/page.tsx` → server-side `redirect()` на новий хаб. Sidebar (`components/molecules/Sidebar.tsx`) скоротив «Курси»+«Бібліотека» в одну «Бібліотека» → `/dashboard/library`. **Cross-linking**: `LibraryLesson` type розширено `slug/courseDocumentId/courseSlug/courseTitle/sectionSlug`; `lib/teacher-library.ts` LIST + DETAIL queries додають `populate[course]` + `fields[slug,sectionSlug]`. **Course editor** (`/dashboard/courses/[id]/edit`) — slug-text-inputs замінено на новий `LessonPickerModal` (multi-select з пошуком, фільтр level, бейджі «У юніті»/«В іншому курсі», disabled для already-assigned); чипи уроків показують title (lookup через `fetchLessonsCached`) + slug + reorder/remove. **Lesson editor** back-link обробляє новий бейдж «У курсі: <Title> · <Section>» (lazy-fetch course detail для назви секції). Dual-truth `lessonSlugs[]` ↔ `lesson.{course,sectionSlug}` лишається — picker пише тільки в `course.sections[].lessonSlugs[]`, повна міграція до single-source (drop `lessonSlugs[]`, write `lesson.sectionSlug + orderIndex`) deferred як окремий чанк (вимагає Strapi-міграції + backfill-скрипту). Frontend `tsc --noEmit` + `eslint` чисті на всі змінені файли (pre-existing `any`-warnings у `teacher-library.ts` без регресій).
