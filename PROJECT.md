...
- ~~`@/mocks/user`~~ — видалено (Phase A6). Типи перенесено у `@/lib/types`; `useKidsIdentity()` — єдиний seam для Kids Zone до Phase B.
- `@/mocks/lessons/*` — 12 файлів: 7 rich-lesson TS дата + 11 step-компонентів у `components/lesson/` (тільки type-only імпорт `LessonData`/`Step*`). Видаляється у Phase **A8**.
...
- [ ] **A7** Запустити `npm run import-mocks` проти staging + admin-spot-check. Виконується руками людиною з доступом до Railway; скрипт готовий, типізація чиста. Runbook нижче.
- [ ] **A8** Мігрувати `frontend/mocks/lessons/*.ts` (rich LessonEngine-структура — theory/mcq/fill-blank/word-order/match-pairs/translate/image/video/reading steps) у Strapi. `LessonPage` (`app/courses/[courseSlug]/lessons/[lessonSlug]/page.tsx`) зараз читає hardcoded TS реєстр — це блокер для інтерактивного плеєра на живих даних. Два варіанти: (a) `lesson.content` → JSON-поле із типізацією `LessonData` на FE; (b) розширити `exercise` component під всі step-типи. Варіант (a) простіший, варіант (b) дає admin-UI.

**Runbook — A7 (import-mocks → staging)**

```bash
# 1. Проти локального Postgres (санітарний прогін)
cd backend
cp .env.example .env            # якщо ще не створено
export DATABASE_URL="postgres://..."   # або що у .env
export JWT_SECRET="..." APP_KEYS="..." ADMIN_JWT_SECRET="..." API_TOKEN_SALT="..." TRANSFER_TOKEN_SALT="..."
npm run import-mocks

# 2. Проти Railway staging (після санітарного прогону)
railway run --service backend npm run import-mocks
# або:
DATABASE_URL="$(railway variables -s backend | grep DATABASE_URL | cut -d= -f2-)" npm run import-mocks

# 3. Admin-spot-check у Strapi адмінці:
# - Content → Course: 6+ записів зі slug/title/teacher
# - Content → Lesson: лесони прив'язані до курсів, exercises заповнені
# - Content → Session: sessions мають startAt в ISO, teacher+course
# - Content → Teacher profile: 3+ вчителі з publicSlug
# - Users → 3+ seed-юзерів @placeholder.englishbest.app (тестові паролі — у логах скрипта)

# 4. FE спот-чек (`NEXT_PUBLIC_API_BASE_URL` дивиться на staging):
# - /courses — рендерить список
# - /courses/<slug> — hero + sections + teacher
# - /courses/<slug>/lessons/<slug> — поки рендерить mock (до A8)
```

Rollback: скрипт ідемпотентний (dedupe по slug/publicSlug/email+startAt). Якщо треба скасувати — видалити записи через Admin UI (Strapi Documents API поважає каскад teacher→user-profile→user).

**Acceptance (Phase A):** Courses page, Lessons catalog, Calendar, Profile, Home dashboard рендерять живі дані зі Strapi; `grep -R "from '@/mocks" frontend/` показує тільки `mocks/*.json` (seed-дані) та `mocks/lessons/*.ts` (до A8); e2e smoke працює без `/api/mock`.
