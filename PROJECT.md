...
- **2026-04-28** Read-switch (stage C.1) — `lib/normalize.ts.normalizeCourse` + `lib/teacher-courses.ts.normalizeDetail` тепер виводять `section.lessonSlugs[]` з populated `lessons` relation (group by `sectionSlug`, sort by `orderIndex`); якщо relation для секції порожня — fallback на legacy `course.sections[].lessonSlugs[]` component-array. Безпечно для не-backfilled курсів. `COURSE_POPULATE` (`lib/api.ts`) + teacher `DETAIL_QUERY` розширено `populate[lessons][fields]=slug,sectionSlug,orderIndex`. Public-pages (`/library`, `/library/[programSlug]`, `/(kids)/kids/library/[id]`) і course editor читають той самий derived `lessonSlugs[]` — жодних змін у consumer code. Stage C.2 (drop schema field + dual-write code) лишається deferred — спрацює лише після того, як backfill виконаний на prod і derive shape підтверджено в логах/QA.
