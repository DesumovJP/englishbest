...
- [ ] **A4** `lib/config.ts` + `lib/api.ts` → писати в реальні Strapi-ендпоінти (courses/lessons/sessions/reviews/user-progress/teacher-profile). `/api/mock/**` — gate за flag `NEXT_PUBLIC_USE_MOCK` (за замовчуванням off у prod).
