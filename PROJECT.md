...
- [ ] **A6** Видалити `@/mocks/user` з 9 файлів (не 25 — initial count був завищений). Розбито на три підкроки:
  ...
  - [ ] **A6.2** Замінити `mockKidsUser` у 6 файлах (dashboard/school/characters/library-[id]/shop/welcome). Поля що реально читаються: `name`, `coins`, `level` — гідруємо з `useSession()` + `kids-store` (IndexedDB, autoritet до Phase B).
  - [ ] **A6.3** Видалити `frontend/mocks/user.ts` (нуль залишкових імпортів).
