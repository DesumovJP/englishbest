...
- [ ] **A4.2** Видалити `app/api/mock/**` + `lib/mockClient.ts` + мігрувати залишкових споживачів (`CoursePage`, `CalendarView`, `courses/[courseSlug]/page.tsx`) на `lib/api.ts`. Видалити `frontend/mocks/*.json` після підтвердження, що `import-mocks` виконаний проти staging.
