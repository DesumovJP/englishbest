'use client';
/**
 * ChatPanel — shared chat surface for staff (/dashboard/chat) and kids
 * (/kids/chat). Owns thread list + thread view + composer + 10s polling.
 *
 * Variants only affect chrome:
 *   - 'staff' — wraps in a bordered card and shows the "Написати всім"
 *               mass-message button. Kept for the existing dashboard layout.
 *   - 'kids'  — full-bleed (no border, fills parent). Mass-message hidden;
 *               kids never broadcast.
 *
 * Sizing: the panel is `flex flex-col h-full min-h-0`; the parent decides
 * the height. Staff page wraps in a fixed h-calc to fit under the topbar;
 * the kids page lets it expand inside KidsPageShell's flex-1 main.
 */
...
export type ChatPanelVariant = 'staff' | 'kids';
...
function ChatPanelInner({ variant }: ChatPanelInnerProps) {
  ...
  const showMassButton = variant === 'staff';
    ...
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      {!isKids && (
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-border flex-shrink-0">
          ...
          {showMassButton && <Button size="sm" onClick={() => setMassOpen(true)}>Написати всім</Button>}
        </div>
      )}
      ...
      {showMassButton && <MassMessageModal open={massOpen} onClose={() => setMassOpen(false)} />}
    </div>
  );
}
...
export function ChatPanel({ variant }: ChatPanelProps) {
  return (
    <Suspense>
      <ChatPanelInner variant={variant} />
    </Suspense>
  );
}
