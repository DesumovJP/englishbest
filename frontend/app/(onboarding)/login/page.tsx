...
const ACCOUNT_TYPES: AccountConfig[] = [
  ...
  { id: 'admin',   emoji: '🛡️',        label: 'Admin',   sublabel: 'Адміністратор',     redirect: '/dashboard/admin',      tone: 'purple',  wip: true },
  { id: 'parent',  emoji: '👨‍👩‍👧',    label: 'Parent',  sublabel: 'Батьки',            redirect: '/dashboard/parent',     tone: 'accent',  wip: true },
];
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        {ACCOUNT_TYPES.map(type => {
          ...
          const wip = type.wip === true;
          return (
            <div key={type.id}
              className={`flex flex-col gap-2 rounded-2xl p-4 text-left bg-white border-2 border-border shadow-card ${wip ? 'opacity-70' : ''}`}>
              <div className="flex items-start justify-between gap-2">
                <span className="text-[28px]">{type.emoji}</span>
                {wip && (
                  <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md bg-surface-muted text-ink-muted">
                    В розробці
                  </span>
                )}
              </div>
              ...
              <button
                onClick={() => !wip && onDemo(type.id)}
                disabled={wip}
                className={`mt-1 w-full rounded-xl font-black text-white py-2 text-xs transition-transform ${
                  wip
                    ? 'bg-ink-faint cursor-not-allowed'
                    : `active:translate-y-0.5 active:shadow-none ${tone.btn}`
                }`}
              >
                {wip ? 'Скоро' : 'Demo →'}
              </button>
