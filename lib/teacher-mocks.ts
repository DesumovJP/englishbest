...
export const LEVEL_STYLES: Record<Level, string> = {
  ...
  A1: 'bg-accent/15 text-accent-dark',
  A2: 'bg-accent/25 text-accent-dark',
  B1: 'bg-success/10 text-success-dark',
  B2: 'bg-purple/10 text-purple-dark',
  C1: 'bg-secondary/15 text-secondary-dark',
};

export const LESSON_STATUS_STYLES: Record<LessonStatus, { label: string; cls: string; dot: string }> = {
  scheduled:     { label: 'Заплановано', cls: 'bg-secondary/15 text-secondary-dark', dot: 'bg-secondary' },
  'in-progress': { label: 'Зараз',       cls: 'bg-accent/15 text-accent-dark',       dot: 'bg-accent animate-pulse' },
  done:          { label: 'Проведено',   cls: 'bg-primary/10 text-primary-dark',     dot: 'bg-primary' },
  cancelled:     { label: 'Скасовано',   cls: 'bg-surface-muted text-ink-muted',     dot: 'bg-border' },
};
...
export const HOMEWORK_STATUS_STYLES: Record<HomeworkStatus, { label: string; cls: string }> = {
  ...
  'in-progress': { label: 'В процесі',    cls: 'bg-accent/15 text-accent-dark' },
  submitted:     { label: 'На перевірці', cls: 'bg-secondary/15 text-secondary-dark' },
  reviewed:      { label: 'Перевірено',   cls: 'bg-primary/10 text-primary-dark' },
  returned:      { label: 'Повернуто',    cls: 'bg-danger/10 text-danger-dark' },
  overdue:       { label: 'Прострочено',  cls: 'bg-danger/15 text-danger-dark' },
};
