    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-ink-muted hover:text-ink hover:bg-surface-muted transition-colors">
          ...
          <div className="flex flex-col divide-y divide-border">
            {TRANSACTIONS.map(t => (
              <div key={t.id} className="flex items-center justify-between gap-4 py-3.5">
                ...
                <span className={`text-sm font-black flex-shrink-0 ${t.positive ? 'text-primary-dark' : 'text-ink'}`}>{t.amount}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="px-6 py-4 border-t border-border flex-shrink-0">
          <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-surface-muted text-sm font-bold text-ink hover:bg-border transition-colors">Закрити</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
        ...
        <div className="bg-white rounded-2xl border border-border p-6 flex items-center gap-5">
          <div className="relative flex-shrink-0">
            ...
            <img src={USER.photo} alt={USER.name} className="w-20 h-20 rounded-2xl object-cover" referrerPolicy="no-referrer" />
        ...
        <div className="bg-gradient-to-br from-primary to-primary-dark rounded-2xl p-6 flex flex-col gap-4">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              ...
              <button onClick={() => setShowPayments(true)} className="text-xs text-white/70 hover:text-white underline underline-offset-2 mt-1 transition-colors">
                Історія →
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card
          ...
          action={
            <button
              onClick={handleSave}
              className={`text-xs font-black px-4 py-2 rounded-xl transition-all ${saved ? 'bg-primary/10 text-primary-dark' : 'bg-gradient-to-br from-primary to-primary-dark text-white hover:opacity-90'}`}
            >
      ...
      <div className="bg-white rounded-2xl border border-border overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-3">
          <button className="flex items-center justify-between w-full px-4 py-3 rounded-xl border border-border hover:bg-surface-muted transition-colors text-left group">
            ...
            <svg className="w-4 h-4 text-ink-muted group-hover:text-ink transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" aria-hidden><path d="M9 18l6-6-6-6" /></svg>
          </button>
          <div className="border-t border-border pt-3 flex flex-col gap-2">
            <button className="flex items-center gap-3 w-full px-4 py-3 rounded-xl border border-border hover:bg-surface-muted transition-colors text-left text-sm font-semibold text-ink-muted hover:text-ink">
              <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" aria-hidden><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 17l5-5-5-5M21 12H9" /></svg>
              Вийти з акаунту
            </button>
            <button className="flex items-center gap-3 w-full px-4 py-3 rounded-xl border border-danger/30 hover:bg-danger/5 transition-colors text-left text-sm font-semibold text-danger">
              <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" aria-hidden><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2" /></svg>
              Видалити акаунт
